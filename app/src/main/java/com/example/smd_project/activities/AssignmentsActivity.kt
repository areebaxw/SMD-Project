            ...
            try {
                ...
                if (cachedMarks.isNotEmpty() || allCourses.isNotEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        setupCourseFilter()
                        Toast.makeText(this@AssignmentsActivity, "Showing cached data (offline)", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@AssignmentsActivity, "No cached data available. Please connect to internet.", Toast.LENGTH_LONG).show()
                    }
                    return@withContext
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@AssignmentsActivity, "Error loading offline data", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
