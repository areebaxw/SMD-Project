...
class QuizActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        // Highlight current tab - Quizzes
        findViewById<TextView>(R.id.tab_quizzes).apply {
    ...
    private fun setupRecyclerView() {
        ...
        recyclerView.apply {
            ...
            adapter = this@QuizActivity.adapter
        }
    }
    ...
    private fun loadMarksData() {
        lifecycleScope.launch {
            try {
                if (NetworkUtils.isOnline(this@QuizActivity)) {
            ...
            try {
                ...
                if (cachedMarks.isNotEmpty() || allCourses.isNotEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        setupCourseFilter()
                        Toast.makeText(this@QuizActivity, "Showing cached data (offline)", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@QuizActivity, "No cached data available", Toast.LENGTH_LONG).show()
                    }
                    return@withContext
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@QuizActivity, "Error loading offline data", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun filterMarksByCourseAndType() {
        val filtered = allMarks.filter { mark ->
            ...
            mark.evaluation_type?.contains("Quiz", ignoreCase = true) == true
        }
        adapter.updateMarks(filtered)
        
        if (filtered.isEmpty()) {
            Toast.makeText(this, "No quizzes for this course", Toast.LENGTH_SHORT).show()
        }
    }
}
