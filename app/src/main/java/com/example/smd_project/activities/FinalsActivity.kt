...
class FinalsActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        // Highlight current tab - Finals
        findViewById<TextView>(R.id.tab_finals).apply {
    ...
    private fun setupRecyclerView() {
        recyclerView = findViewById(R.id.rvFinals)
        adapter = StudentMarkDisplayAdapter(emptyList())
        
        recyclerView.apply {
            layoutManager = LinearLayoutManager(this@FinalsActivity)
            adapter = this@FinalsActivity.adapter
        }
    }
    ...
    private fun loadMarksData() {
        lifecycleScope.launch {
            try {
                if (NetworkUtils.isOnline(this@FinalsActivity)) {
    ...
    private suspend fun cacheMarks(marks: List<Mark>) {
        withContext(Dispatchers.IO) {
            try {
                val database = AppDatabase.getDatabase(this@FinalsActivity)
            ...
            try {
                ...
                if (cachedMarks.isNotEmpty() || allCourses.isNotEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        setupCourseFilter()
                        Toast.makeText(this@FinalsActivity, "Showing cached data (offline)", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@FinalsActivity, "No cached data available", Toast.LENGTH_LONG).show()
                    }
                    return@withContext
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@FinalsActivity, "Error loading offline data", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun filterMarksByCourseAndType() {
        val filtered = allMarks.filter { mark ->
            ...
            mark.evaluation_type?.contains("Final", ignoreCase = true) == true
        }
        adapter.updateMarks(filtered)
        
        if (filtered.isEmpty()) {
            Toast.makeText(this, "No finals for this course", Toast.LENGTH_SHORT).show()
        }
    }
}
