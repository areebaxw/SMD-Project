...
class SessionalsActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        // Highlight current tab - Sessionals
        findViewById<TextView>(R.id.tab_sessionals).apply {
    ...
    private fun setupRecyclerView() {
        recyclerView = findViewById(R.id.rvSessionals)
        adapter = StudentMarkDisplayAdapter(emptyList())
        
        recyclerView.apply {
            layoutManager = LinearLayoutManager(this@SessionalsActivity)
            adapter = this@SessionalsActivity.adapter
        }
    }
    ...
    private fun loadMarksData() {
        lifecycleScope.launch {
            try {
                if (NetworkUtils.isOnline(this@SessionalsActivity)) {
    ...
    private suspend fun cacheMarks(marks: List<Mark>) {
        withContext(Dispatchers.IO) {
            try {
                val database = AppDatabase.getDatabase(this@SessionalsActivity)
            ...
            try {
                ...
                if (cachedMarks.isNotEmpty() || allCourses.isNotEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        setupCourseFilter()
                        Toast.makeText(this@SessionalsActivity, "Showing cached data (offline)", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@SessionalsActivity, "No cached data available", Toast.LENGTH_LONG).show()
                    }
                    return@withContext
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@SessionalsActivity, "Error loading offline data", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun filterMarksByCourseAndType() {
        val filtered = allMarks.filter { mark ->
            ...
            mark.evaluation_type?.contains("Sessional", ignoreCase = true) == true
        }
        adapter.updateMarks(filtered)
        
        if (filtered.isEmpty()) {
            Toast.makeText(this, "No sessionals for this course", Toast.LENGTH_SHORT).show()
        }
    }
}
